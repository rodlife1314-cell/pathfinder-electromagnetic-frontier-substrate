...
export const TransportMediumView: React.FC = () => {
  ...
  const [civilizationPhase, setCivilizationPhase] = useState<'NATURAL_TOPOLOGY' | 'HYBRID_CANAL' | 'ENGINEERED_GRID'>('ENGINEERED_GRID');
  ...
  const [customEigenmodeScore, setCustomEigenmodeScore] = useState<number>(0.85);
  ...
  const activeVessel: VesselConfiguration = useMemo(() => {
    ...
    return {
      ...
      eigenmodeSynthesisScore: customEigenmodeScore
    };
  }, [selectedVesselId, customVesselImpedance, customEigenmodeScore]);
  ...
  const handleVesselSelect = (vId: string) => {
    ...
    if (v) {
      ...
      setCustomEigenmodeScore(v.eigenmodeSynthesisScore);
    }
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-zinc-100">
      ...
      <div className="p-6 rounded-xl bg-zinc-900 border border-zinc-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-zinc-400 max-w-3xl leading-relaxed">
            ...
            <strong className="text-amber-300 font-mono">T = R ⊕ V</strong>, where velocity and throughput are properties of the{' '}
            <strong className="text-cyan-300">Ship–Rail Pair</strong>, not the isolated vehicle.
          </p>
        </div>

        {/* System Equation Badge */}
        <div className="p-3.5 rounded-lg bg-zinc-950 border border-zinc-800 font-mono text-xs text-center shrink-0 space-y-1">
          <div className="text-[10px] text-zinc-500 uppercase">CIVILISATION EQUATION</div>
      ...
      <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-3 font-mono text-xs">
        <div className="flex items-center justify-between">
          <span className="text-zinc-400 uppercase font-semibold text-[11px] flex items-center space-x-2">
          ...
          <button
            ...
            className={`p-3 rounded-lg border text-left transition-all ${
              civilizationPhase === 'NATURAL_TOPOLOGY'
                ? 'bg-zinc-950 border-amber-500/80 shadow-md'
          ...
          <button
            ...
            className={`p-3 rounded-lg border text-left transition-all ${
              civilizationPhase === 'HYBRID_CANAL'
                ? 'bg-zinc-950 border-cyan-500/80 shadow-md'
                : 'bg-zinc-950/60 border-zinc-800/80 hover:border-zinc-700'
            }`}
          >
          ...
          <button
            ...
            className={`p-3 rounded-lg border text-left transition-all ${
              civilizationPhase === 'ENGINEERED_GRID'
                ? 'bg-zinc-950 border-emerald-500/80 shadow-md'
                : 'bg-zinc-950/60 border-zinc-800/80 hover:border-zinc-700'
            }`}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 p-5 rounded-xl bg-zinc-900 border border-zinc-800 flex flex-col items-center justify-between space-y-4">
          ...
          <div className="relative w-full max-w-[560px] aspect-[16/10] bg-zinc-950 rounded-xl border border-zinc-800 overflow-hidden shadow-inner flex items-center justify-center">
            <svg viewBox="0 0 850 480" className="w-full h-full select-none">
              ...
              {DEFAULT_RAILS.map((rail) => {
                ...
                const strokeColor = rail.type === 'SYNCHRONOUS_EXPRESS'
                  ? '#10b981'
          ...
          <div className="w-full flex flex-wrap items-center gap-1.5 pt-2 border-t border-zinc-800 text-xs font-mono">
            <span className="text-zinc-500 mr-2 text-[10px]">CORRIDOR SELECTION:</span>
            {DEFAULT_RAILS.map(rail => (
              <button
                key={rail.id}
                onClick={() => handleRailSelect(rail.id)}
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="p-5 rounded-xl bg-zinc-900 border border-zinc-800 space-y-4 font-mono text-xs">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-2">
              <span className="font-bold text-zinc-200 uppercase tracking-wider flex items-center space-x-1.5">
            ...
            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 space-y-2 text-center">
              ...
              <div className="text-3xl font-extrabold text-emerald-400 tracking-tight">
                {(dynamics.maxAttainableVelocityC * 100).toFixed(2)}% <span className="text-lg font-mono text-zinc-400">c</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2.5 text-[11px]">
              <div className="p-2.5 rounded bg-zinc-950 border border-zinc-800">
                <span className="text-[9px] text-zinc-500 block">COUPLING PRODUCT (η)</span>
                <span className="text-cyan-400 font-bold text-sm">
                  {(dynamics.systemCouplingProduct * 100).toFixed(1)}%
                </span>
                <span className="text-[9px] text-zinc-500 block mt-0.5">Z_match × σ_phase</span>
              </div>

              <div className="p-2.5 rounded bg-zinc-950 border border-zinc-800">
                <span className="text-[9px] text-zinc-500 block">EFFECTIVE THRUST</span>
                <span className="text-amber-400 font-bold text-sm">
                  {dynamics.effectiveThrustKiloNewtons.toFixed(1)} kN
                </span>
                <span className="text-[9px] text-zinc-500 block mt-0.5">Coupled Rail Momentum</span>
              </div>

              <div className="p-2.5 rounded bg-zinc-950 border border-zinc-800">
                <span className="text-[9px] text-zinc-500 block">RETURN LOSS (REFLECTION)</span>
              ...
              <div className="p-2.5 rounded bg-zinc-950 border border-zinc-800">
                <span className="text-[9px] text-zinc-500 block">DISSIPATIVE THERMAL LOSS</span>
                <span className="text-red-400 font-bold text-sm">
                  {dynamics.energyDissipationLossMw.toFixed(0)} MW
                </span>
                <span className="text-[9px] text-zinc-500 block mt-0.5">Uncoupled Radiation</span>
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {DEFAULT_VESSELS.map(v => (
                  ...
                  >
                    ...
                    <div className="text-[9px] text-zinc-500">{v.classType.replace('_', ' ')}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2 border-t border-zinc-800 text-[11px]">
              <div>
      ...
      <div className="p-5 rounded-xl bg-zinc-900 border border-zinc-800 space-y-4">
        <div className="flex items-center justify-between pb-2 border-b border-zinc-800 text-xs font-mono">
          <span className="font-bold text-zinc-200 uppercase tracking-wider flex items-center space-x-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
            <span>CRITICAL DOCTRINE PROOF: THE SHIP–RAIL PAIR PARADOX</span>
          </span>
          <span className="text-zinc-500">Infrastructure determines capability ceiling</span>
        </div>
          ...
          {comparativePairs.map((pair, idx) => (
            ...
            >
              <div className="flex justify-between items-center border-b border-zinc-800/80 pb-2">
                <span className="font-bold text-zinc-100">{pair.tag}</span>
                <span className="text-[10px] text-zinc-500">Pair #{idx + 1}</span>
              </div>
              ...
              <div className="p-2.5 rounded bg-zinc-900/90 border border-zinc-800 space-y-1">
                <div className="flex justify-between text-[10px] text-zinc-400">
                  <span>Rail:</span>
                  <span className="text-zinc-200 truncate max-w-[160px]">{pair.rail}</span>
                </div>
                <div className="flex justify-between text-[10px] text-zinc-400">
                  <span>Vessel:</span>
                  <span className="text-zinc-200 truncate max-w-[160px]">{pair.vessel}</span>
                </div>
              </div>

              {/* Realized Speed Comparison */}
              <div className="flex justify-between items-center pt-1">
                <span className="text-[10px] text-zinc-500 uppercase">Max Velocity:</span>
                <span className={`text-base font-bold ${
                  idx === 1 ? 'text-amber-400' : idx === 2 ? 'text-emerald-400' : 'text-zinc-400'
                }`}>
                  {(pair.result.maxAttainableVelocityC * 100).toFixed(2)}% c
                </span>
              </div>

              <p className="text-[11px] text-zinc-400 italic pt-1 border-t border-zinc-800/60 leading-relaxed">
                {pair.note}
              </p>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-xl bg-zinc-900 border border-zinc-800 space-y-3 font-mono text-xs">
          ...
          <ul className="space-y-2 text-zinc-300 text-[11px]">
            <li className="p-2 rounded bg-zinc-950 border border-zinc-800 flex justify-between items-center">
            ...
            <li className="p-2 rounded bg-zinc-950 border border-zinc-800 flex justify-between items-center">
              ...
              <span className="text-zinc-400 font-bold">Kuiper & Sol Buffers</span>
            </li>
          </ul>
        </div>
        ...
        <div className="p-5 rounded-xl bg-zinc-900 border border-zinc-800 space-y-3 font-mono text-xs">
          ...
          <ul className="space-y-2 text-zinc-300 text-[11px]">
            ...
            <li className="p-2 rounded bg-zinc-950 border border-zinc-800 flex justify-between items-center">
              ...
              <span className="text-zinc-400 font-bold">Γ_δ Transverse Bending</span>
            </li>
          </ul>
        </div>
      </div>
